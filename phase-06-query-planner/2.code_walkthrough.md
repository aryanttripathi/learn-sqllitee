...
Check it against the table in `1.knowledge.md` §6.2: `sqlite3LogEst(10)` → normalizes to
...
- `rSize + 16` — `rSize` is `LogEst(rows)`; `+16` multiplies by ~3. So **a full scan costs
  ...
  `1.knowledge.md` §6.2, and here is where it lives.
...
10. Find `sqlite3DefaultRowEst()` and confirm the five default constants from
    `1.knowledge.md` §6.2 against the source.
